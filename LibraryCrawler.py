...
def crawlerFun():
    ...
    # driver = webdriver.Chrome(options=chrome_options)  这样填是从环境变量中运行chromedriver
    driver = webdriver.Chrome(options=chrome_options,service=service) # 这样填是指定路径运行chromedriver
    ...
    # close browser
    driver.quit()
    logfile=open("./log",mode="a+",encoding="utf8")
    logfile.write("Time:$s\nCrawler Successfully!\n\n"%datetime.now())
    logfile.flush()
    logfile.close()

scheduler=BlockingScheduler()
scheduler.add_job(crawlerFun,"interval", hours=1)
scheduler.start()